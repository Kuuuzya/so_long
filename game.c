...
/*   Updated: 2024/07/25 21:23:01 by skuznets         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void free_sources(t_data *data)
{
    ...
    if (data->map)
        free_map(data->map);
    if (data->mlx)
        free(data->mlx); // Освобождение mlx
}

void end_game(t_data *data, const char *message)
{
    if (data->game_over) return;  // Убедись, что end_game не вызывается повторно
...
int load_textures(t_data *data)
{
    ...
    if (!data->wall_img || !data->floor_img || !data->player_img[0] || !data->player_img[1] || 
        !data->exit_img || !data->collectible_img || !data->enemy_img)
    {
        ft_printf("Error loading one of the textures\n");
        // Освобождение уже загруженных текстур
        free_sources(data);
        return (0);
    }
    return (1);
...
void move_enemy_random(t_data *data, int ex, int ey)
{
    ...
    if (valid_count > 0)
    {
        ...
        if (data->map[new_y][new_x] == 'P')
            end_game(data, "Вы проиграли");  // Устанавливаем флаг game_over
        else
        {
